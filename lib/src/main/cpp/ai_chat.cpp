...
static llama_context *init_context(
    ...
    const int auto_threads = std::max(N_THREADS_MIN, std::min(N_THREADS_MAX,
                                                        (int) sysconf(_SC_NPROCESSORS_ONLN) -
                                                        N_THREADS_HEADROOM));
    const int n_threads = requested_threads > 0 ? requested_threads : auto_threads;
...
Java_com_arm_aichat_internal_InferenceEngineImpl_prepare(
    ...
    auto *context = init_context(g_model, g_context_size, threads);
    if (!context) { return 1; }
    g_context = context;
...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_arm_aichat_internal_InferenceEngineImpl_backendNames(JNIEnv *env, jobject /*unused*/) {
